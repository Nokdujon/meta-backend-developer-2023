...
def test_length(input_value):
    ...
    assert spell_check.word_count(input_value) < 10
    assert spell_check.char_count(input_value) < 50
...
def test_struc(input_value):
    ...
    """
    ### WRITE SOLUTION CODE HERE
    assert True if spell_check.first_char(input_value).isupper() else False
    assert True if spell_check.last_char(input_value) == "." else False
